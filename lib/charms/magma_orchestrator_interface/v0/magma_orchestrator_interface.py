...
class OrchestratorRequires(Object):
    ...
    def _on_relation_changed(self, event: RelationChangedEvent) -> None:
        ...
        if not relation.app:
            logger.warning(
                f"No remote application in relation: {self.relationship_name}")
            return
        if not event.app:
        ...
        self.on.orchestrator_available.emit(
            ...
            orchestrator_port=int(
                remote_app_relation_data["orchestrator_port"]),
            bootstrapper_address=remote_app_relation_data["bootstrapper_address"],
            bootstrapper_port=int(
                remote_app_relation_data["bootstrapper_port"]),
...
class OrchestratorProvides(Object):
    ...
    def set_orchestrator_information(
        ...
        if not self.charm.unit.is_leader():
            raise RuntimeError(
                "Unit must be leader to set application relation data.")
        ...
        if not relations:
            raise RuntimeError(
                f"Relation {self.relationship_name} not yet created")
